function [J, J_noiseremoved] = MSER_feature_extraction(J_feature, fps, nbins, movmean_windowratio)
...
RangeOfArea = zeros(numFrames, 2); % [min, max]
for i = 1 : numFrames
    currJ = J_feature{i};
    if currJ.Count>=2
...
if numel(X1) == nbins
    ...
    for k = 1 : numFeatures
        V = J(:, k);
        V_filtered = ReduceSignalNoise(V', fps, movmean_windowratio);

function [curvdatafiltered, dKdt_data, angledatafiltered, lendata, inconst, fullnewdirname, w_diam, areadata] = WORMSHAPE_MAINCALCULATION(vidObj, options, resizefactor, tscale)
...
mov_quality = .9;
if issavefiles
    fullnewdirname = fullfile(pathname,strrep(filename,'.avi',sprintf('_worm%d_%d-%d',wormlabel,istart,iend)));
    mkdir(fullnewdirname);
end

if domovie   % MOV
    savefname= strrep(filename,'.avi',sprintf('_%d-%d.mov',istart,iend));
...
%     image(img);
    ...
    bkg = imread(strrep(filename, 'bkgsubtracted.avi', 'background.bmp'));
    figure(2); clf
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% to release
    bkgx2 = size(bkg,2); bkgy2 = size(bkg,1); %%%%%%%%%%%%%
    bkgx2 = floor(bkgx2);
    bkgy2 = floor(bkgy2);
...
%%%%%%%%%%%%%%%%%%%%%%%
iscontinue = 0; % Should be iscontinue = 1;
%%%%%%%%%%%%%%%%%%%%%%%
while iscontinue == 1
    figure(2); hold on;
    title('Indicate none-worm ROI')
    answer = length(questdlg('Continue to remove none-worm?', 'BKG modification', 'Continue', 'No', 'No'));
    if answer == 8
        iscontinue = 1;
        figure(2); hold on;
        [bw_nw, xi, yi] = roipoly;
        patch(xi, yi, 'g', 'FaceColor', 'none','EdgeColor','r', 'LineStyle',':');
...
%     
    ...
    corner_mean(j) = mean(mean(imgcrop(1:end,1:end)));
    
    
%     figure(1); hold off;
%     imagesc(imgcrop, [5 250]); hold on;
%     text(20,20,[num2str(1*double(i-istart)/fps*tscale, '%.2f') ' s'], 'Color', 'w');
%     axis image;
...
%         text(10,10,'zoom in, press any key', 'Color', 'white');
%         zoom on; zoom off;
%         text(10,10,'zoom in, press any key', 'Color', 'black');
%         
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% to release
        ...
        if domovie
            ...
            MakeQTMovie('size', mov_size_multiplier*[size(img,2) size(img,1)]);
...
%     plot(path2_rescaled(end,2),path2_rescaled(end,1), 'og'); hold on;
...
%     figure(1); axis image;
%     fnplt(cv, '-g'); hold on;
%     plot(xy(1,51),xy(2,51),'or');hold on ;% centre point of cv

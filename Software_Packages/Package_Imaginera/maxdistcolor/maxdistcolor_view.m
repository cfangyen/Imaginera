...
%
% See also MAXDISTCOLOR MAXDISTCOLOR_DEMO SRGB_TO_OKLAB
...
pAxS(9) = struct('title','RGB Gamut Point Cloud',...
...
%
	function fPointCloud(axh) % Show the RGB gamut using a point cloud.
		...
		% Plot point cloud:
		nmr = size(UCS,1);
		I = unique([round(linspace(1,nmr,12345)),ceil(nmr*rand(1,23456))]);
